...
/**
 * Description of InnValidator
 *
 * @author exru
 */

class Ks extends Validator {
    ...
    private function _getKsFactor($value, $factor) {
        $sum = 0;
        $number = count($factor);
        for ($i = 0; $i != $number; $i++) {
    ...
    public function validateAttribute($model, $attribute) {
        if (!$model->hasAttribute($this->bikAttribute) || $model->{$this->bikAttribute} == '') {
        ...
        if (!$this->_getKsFactor('0'.substr($model->{$this->bikAttribute}, 4, 2).$model->{$attribute}, self::$FACTOR)) {
